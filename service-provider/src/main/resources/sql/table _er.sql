...
CREATE TABLE customer(
  ...
  wallet_id INT NOT NULL COMMENT '用户钱包id',
  ...
  KEY idx_id(id),
  CONSTRAINT FOREIGN KEY (wallet_id) REFERENCES wallet (id),
  CONSTRAINT FOREIGN KEY (addr_detail_id) REFERENCES addr_detail (id)
) DEFAULT CHARSET = utf8
  COMMENT ='普通用户数据表';
...
CREATE TABLE collect_range(
  ...
  CONSTRAINT FOREIGN KEY (addr_detail_id) REFERENCES addr_detail (id) 
    ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT FOREIGN KEY (collector_id) REFERENCES collector (id)
    ON DELETE CASCADE ON UPDATE CASCADE
) DEFAULT CHARSET = utf8
...
CREATE TABLE garbage(
  ...
  PRIMARY KEY (name),
  KEY idx_id(id),
  CONSTRAINT FOREIGN KEY (type_name) REFERENCES garbage_type (name)
) DEFAULT CHARSET = utf8
  COMMENT ='具体废品数据表';
...
CREATE TABLE order_item(
  ...
  CONSTRAINT FOREIGN KEY addr_detail_id REFERENCES addr_detail (id)
    ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT FOREIGN KEY state REFERENCES order_state_enum (state)
    ON DELETE CASCADE ON UPDATE CASCADE
) DEFAULT CHARSET = utf8
  COMMENT ='订单数据表';
...
CREATE TABLE order_detail(
  ...
  KEY id_idx(id),
  CONSTRAINT FOREIGN KEY order_item_id REFERENCES order_item (id) 
  ON DELETE CASCADE ON UPDATE CASCADE
) DEFAULT CHARSET = utf8
...
CREATE TABLE express(
  ...
  PRIMARY KEY (id),
  CONSTRAINT FOREIGN KEY customer_id REFERENCES customer (id),
  CONSTRAINT FOREIGN KEY gift_id REFERENCES gift (id) 
) DEFAULT CHARSET = utf8
COMMENT ='礼品快递信息数据表';
